 ...
 *
 * ELEVEN surfaces re-provide `HostRuntimeContext` in total: the nine through
 ...
 * describe two, the onboarding TOUR, and the session-import DIALOG) plus those
 * two. Anything reading `useHostClient()` / `useAddressableHostId()` beneath any
 * of them gets that surface's host.
 *
 * The tour (`onboarding-page.tsx`) was the first outside
 ...
 *
 * The session-import DIALOG (`session-import-dialog.tsx`) joined for
 ...
 * is why it is written down here where the NEXT re-provider gets added. The
 ...
 * That population is no longer a single surface. EIGHT surfaces re-provide
 * `StreamRuntimeContext` in total: `resource-monitor-popover`, which was once
 * the only one, plus the epic sidebar's file tree, the git diff PANEL and the
 * git diff TILE (two surfaces, not one - a pinned panel body and a canvas
 * tile), the HOST OVERVIEW (`host-settings-panel.tsx`, whose
 * `StreamRuntimeContext.Provider` carries `useScopedStreamBinding`), the
 * onboarding TOUR, `app-status-bar` and the session-import DIALOG. Counted, not
 * ranked: an ordinal per surface is a second place for the arithmetic to be
 * wrong, and this list has already carried one that was.
 * The Overview joined the list for its Data & migration
